...
class TestFollowAPI:
    ...
    def test_follow_get(self, user_client, user, follow_1, follow_2, follow_3):
        ...
        assert (
            len(test_data)
            == Follow.objects.filter(following__username="TestUser").count()
    ...
    def test_follow_search_filter(
        ...
        assert (
            len(response.json()) == follow_user.filter(user=user_2).count()
        ), (
        ...
        assert (
            len(response.json())
            == follow_user.filter(user=another_user).count()
        ), (
            "Проверьте, что при GET запросе с параметром `search` на `/api/v1/follow/` "
            "возвращается список соответствующих подписчиков"
        )
